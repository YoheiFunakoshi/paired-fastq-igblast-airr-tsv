...
- GUIの標準スレッド数: `4`
- GUIの標準IgBLAST batch size: `10000`
...
C領域をユニーク判定に使わなくてもユニーク数が極端に変わらなかった理由は、C領域の違いでユニーク数が減るのは、同じVコール、同じJコール、同じ `junctionAA` / `final_junction_aa` を持つクローンがIGHG1、IGHG2、IGHG3など複数のC領域にまたがっている場合に限られるためです。KKF103hGではその影響が全体として限定的で、V/J/junctionAAベースのユニーク判定ではRG社Excelと近い数になったと解釈しています。
